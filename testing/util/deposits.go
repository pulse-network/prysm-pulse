...
// DeterministicDepositsAndKeys function with added deposit amount param.
func DeterministicDepositsAndKeysWithDepositAmount(numDeposits uint64, depositAmount uint64) ([]*ethpb.Deposit, []bls.SecretKey, error) {
	resetCache()
	lock.Lock()
	defer lock.Unlock()
	var err error
